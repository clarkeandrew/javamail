...
public class SMTPTransport extends Transport {
    ...
    private String logMessage = ""; // message returned in the final 250 response from the SMTP server (useful for getting 'queued as' info)
    ...
    public synchronized void close() throws MessagingException {
        ...
        try {
            if (serverSocket != null) {
                sendCommand("QUIT");
                if (quitWait) {
                    int resp = readServerResponse().code;
                    if (resp != 221 && resp != -1 &&
    ...
    protected void notifyTransportListeners(int type, Address[] validSent,
                                            Address[] validUnsent,
                                            Address[] invalid, Message msg, String log) {
    ...
    protected void rcptTo() throws MessagingException {
        ...
        for (int i = 0; i < addresses.length; i++) {
            ...
            retCode = readServerResponse().code;
            switch (retCode) {
                case 250:
                case 251:
                    valid.add(ia);
    ...
    protected void finishData() throws IOException, MessagingException {
        ...
        this.logMessage = issueSendCommand(".", 250);
    }
    ...
    private void openServer(String host, int port)
        ...
        try {
            ...
            int r = -1;
            if ((r = readServerResponse().code) != 220) {
                serverSocket.close();
                serverSocket = null;
                serverOutput = null;
                serverInput = null;
     ...
     * <p>
     * Returns the message after the response code
     */
    private String issueSendCommand(String cmd, int expect)
            throws MessagingException {
        sendCommand(cmd);
        ...
        // throw the exception, notifying the client of the response
        int ret;
        ServerResponse serverResponse = readServerResponse();
        if ((ret = serverResponse.code) != expect) {
    ...
    public synchronized int simpleCommand(String cmd)
            throws MessagingException {
        sendCommand(cmd);
        return readServerResponse().code;
    }
     ...
     * @since JavaMail 1.4.1
     */
    protected ServerResponse readServerResponse() throws MessagingException {
        assert Thread.holdsLock(this);
        ...
        if (serverResponse.length() >= 3) {
            try {
                ...
                returnMessage = serverResponse.length() >= 5 ? serverResponse.substring(5) : "";
        ...
        return new ServerResponse(returnCode, returnMessage);
    }
        ...
        private void bdat(byte[] b, int off, int len, boolean last)
            ...
            if (len > 0 || last) {
                try {
                    ...
                    out.write(b, off, len);
                    out.flush();
                    int ret = readServerResponse().code;
                    if (ret != 250)
